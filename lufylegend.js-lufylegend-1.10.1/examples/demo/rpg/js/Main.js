...
init(50,"mylegend",1920,1080,main);
/**层变量*/
//显示进度条所用层
var loadingLayer;
//游戏底层
...
function gameInit(event){
	...
	//添加点击控制事件
	addEventListener(LMouseEvent.MOUSE_DOWN,ondown);
...
function initMap(){	
	...
	if(imageArray == null){
		...
		imageArray = LGlobal.divideCoordinate(bitmapdata.image.width,bitmapdata.image.height,32,32);
	}
}
//添加地图
function addMap(cx,cy){
	...
	//在地图层上，画出15*10的小图片
	for(i=my;i<40 +Math.abs(cy) && i-mapY < map.length;i++){
...
function delMap(){
	...
	for(i=0;i<mapLayer.childList.length;i++){
		...
		if(bitmap.x + mapLayer.x < 0 || bitmap.x + mapLayer.x >= 2050||
				bitmap.y + mapLayer.y < 0 || bitmap.y + mapLayer.y >= 2050){
